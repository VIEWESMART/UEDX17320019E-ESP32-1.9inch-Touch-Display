/* 1.9" GC9A01 + CHSC6540，硬件参考 arduino/lvgl_v8_port */
#include <stdio.h>
#include <string.h>
#include <assert.h>
#include "esp_err.h"
...
void draw_line(int x1, int y1, int x2, int y2) {
    ...
    lv_point_precise_t *points = lv_malloc(2 * sizeof(lv_point_precise_t));
    if (points == NULL) {
        return;
    }
    ...
    lv_obj_set_style_line_color(line, lv_color_make(0, 0, 0), LV_PART_MAIN);
...
void clear_lines(void) {
    for (int i = 0; i < line_count; i++) {
        void *ud = lv_obj_get_user_data(lines[i]);
        if (ud) {
            lv_free(ud);
        }
        lv_obj_del(lines[i]);
...
static void clear_lines_timer_cb(lv_timer_t *t) {
    (void)t;
    clear_lines();
}
    ...
    if (touch_point.x >= 0 && touch_point.y >= 0) {
        ...
        if (touch_point.x >= 0 && touch_point.y >= 0) {
            ...
            if (clear_timer) {
                ...
                /* 约 5s 无触摸后清除画线（与周期 ms 一致） */
                clear_timer = lv_timer_create(clear_lines_timer_cb, 5000, NULL);
            }
        }
...
void return_to_main_screen(void) {
    ...
    in_touch_test_screen = false;
    in_display_test_screen = false;
    lvgl_port_unlock();
}
...
/* 更新显示测试界面颜色 */
void update_display_test_screen(void) {
...
static void display_test_timer_cb(lv_timer_t *t) {
    (void)t;
    update_display_test_screen();
}
    ...
    switch (event) {
        ...
        case BUTTON_LONG_PRESS_START:
            if (in_display_test_screen) {
                ...
                // 启动定时器以更新显示测试界面颜色
                lv_timer_create(display_test_timer_cb, 1000, NULL);
            }
            break;
